...
| `grace_used_at` | `timestamptz` | NULL — коли видано **єдину** пару-сестру у вікні пільги; CHECK `grace_used_at IS NULL OR rotated_at IS NOT NULL` |
| `created_at` | `timestamptz` | NOT NULL, DEFAULT `now()` |
...
**Кожен запис у цю таблицю — видача, ротація, вихід, вихід звідусіль,
деактивація — іде однією транзакцією, яка спершу бере рядок `admins`
власника `FOR UPDATE`.** Це й робить відкликання і поновлення в польоті
послідовними, а не переплетеними; запис, що оминає блокування, повертає
гонку (див. «Ротація refresh» нижче).
...
- Колонка `admin_refresh_tokens.grace_used_at` і CHECK
  `admin_refresh_tokens_grace_after_rotation` — гілка `fix/admin-auth-races`
  змінює лише схему; міграцію генерує `db:generate` під час інтеграції, щоб
  номери паралельних гілок не зіткнулися. Наявні рядки прибирання не
  потребують: нова колонка NULL у всіх.
...
- `401` — токен невідомий, протермінований або **вже витрачений**.
  ...
  крадіжка, і другий випадок дорожчий (FR-012). Виняток — одна пара-сестра
  у вікні пільги (див. «Ротація refresh»); третє пред'явлення того самого
  токена — вже повтор, навіть усередині вікна.
...
- **Ротація refresh** — кожне поновлення видає нову пару і гасить попередню;
  ...
  вже виданий токен. **Рівно одну на витрачений токен**: її забирає умовний
  `UPDATE … SET grace_used_at = now() WHERE grace_used_at IS NULL`, тож
  повтор, що пред'являє токен сто разів за десять секунд, отримує одну пару,
  а не сто. Третє пред'явлення — повтор, ланцюжок відкликається. Панель
  тримає токени в спільному `localStorage`, і невдале поновлення будь-якої
  вкладки однаково розлогінює всі — тож відкликання тут не погіршує
  досвід, а лише гасить пару, яку міг отримати крадій.
- **Ротація — одна транзакція під блокуванням рядка адміна.** bcrypt
  (порівняння з хешем і хеш наступника, ~500 мс) іде **до** транзакції, без
  блокування. Далі в одній транзакції (READ COMMITTED, обрано свідомо —
  блокування рядка вже серіалізує все, що важить, а SERIALIZABLE додав би
  лише повтори на `40001`): `admins … FOR UPDATE` → перевірка `is_active` →
  `UPDATE … SET rotated_at = now() WHERE rotated_at IS NULL AND expires_at >
  now() RETURNING` → інакше заявка на пару-сестру → інакше повтор. Два
  поновлення одного токена більше не виграють обидва ротацію, а відкликання,
  що збіглося з поновленням у польоті, завжди має останнє слово: або
  поновлення чекає і бачить закритий акаунт / зниклий рядок, або
  відкликання — наступний оператор після блокування — бачить і видаляє
  наступника.
- **Деактивація** — `is_active = false` плюс відкликання всіх refresh цього
  адміна, **однією транзакцією під тим самим блокуванням**. Раніше це були
  два автокоміти, і поновлення, що вже пройшло перевірку `is_active`,
  вставляло наступника вже після видалення. Вхід так само перевіряє
  `is_active` ще раз під блокуванням перед записом сесії: деактивація, що
  прийшла під час bcrypt входу, не отримує свіжої сесії слідом. Access живе
  до 15 хвилин; це свідомий компроміс (SC-003).
