...
Лічильник у базі **точний і під паралельними запитами.** Спроба
записується в `admin_login_attempts` як невдала **до** bcrypt, окремим
автокомітом; потім — окремим автокомітом — рахуються невдачі адреси у вікні
разом із нею самою; правильний пароль перевертає свій рядок на
`succeeded = true`. Раніше порядок був зворотний (порахувати → bcrypt ~250 мс
→ записати), і пачка з N паралельних спроб бачила однакове «менше п'яти» й
перевірялась уся. Тепер k-та спроба, що закомітилась, рахує щонайменше k
рядків, тож понад ліміт перевірок у вікні не буває за жодного переплетення;
помилитись лічильник може лише в бік суворості (порахувати сусіда, що ще в
bcrypt). **Не одна транзакція навмисно**: у транзакції рядок лишався б
невидимим для інших спроб до коміту. Блокування чи з'єднання через bcrypt
не тримається. Вікно міряється годинником бази (`now()`), тим самим, що
ставить `created_at`.

Запис спроби більше **не ковтає помилку**: рядок тепер і є лічильником, і
«журнал упав» без цього означало б «ліміт вимкнено». База, що не приймає цей
insert, не прийме й запис сесії.
...
- **Ліміт** — 5 невдалих за 15 хвилин на email **і** на IP. Дві осі, бо одна
  ...
  акаунтах з однієї адреси — різні атаки (FR-006). Лічильник за email
  витримує паралельну пачку спроб — див. `POST /auth/login` вище.
...
Заборони власної зміни мало: **два SUPER_ADMIN, що одночасно знижують чи
деактивують одне одного**, обидва проходили перевірку (кожен бачив іншого
ще при владі) і лишали організацію без жодного. Тепер `PATCH /admins/:id` —
один метод репозиторію (`AdminRepository.updateAccess`) з власною
транзакцією (READ COMMITTED, обрано свідомо):

1. `SELECT … FROM admins WHERE (role = 'super_admin' AND is_active) OR id IN
   (target, actor) ORDER BY id FOR UPDATE` — усі активні SUPER_ADMIN, ціль і
   виконавець; порядок за `id` не дає двом таким транзакціям зайти в
   дедлок.
2. Якщо ціль — активний SUPER_ADMIN і після зміни перестає ним бути, серед
   заблокованих має лишитися **ще щонайменше один** активний SUPER_ADMIN →
   інакше `409` з кодом `admin-auth.last-super-admin`.
3. Виконавець мусить **досі** бути активним SUPER_ADMIN — гард читав роль,
   коли запит прийшов, а пониження могло закомітитись відтоді → інакше `403`
   `admin-auth.forbidden`.
4. Роль і активність пишуться одним `UPDATE`; деактивація в тій самій
   транзакції видаляє всі refresh-токени акаунта.

Невідомий `id` — `404` `admin-auth.admin-not-found`; не-UUID — `400`
(`ParseUUIDPipe`), а не 500 від Postgres. Обмеженням у базі цей інваріант не
виражається (він про множину рядків, не про один), тож захист — блокування
рядків, а не тригер.
