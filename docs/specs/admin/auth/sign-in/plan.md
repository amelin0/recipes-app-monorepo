...
Indexes:
- `admin_refresh_tokens_admin_idx` on `(admin_id)` — вихід і деактивація
  відкликають усе за адміном (FR-007, FR-008).
- `admin_refresh_tokens_chain_idx` on `(chain_id)` — виявлення повторного
...
- `0012_admins_and_admin_sessions.sql` — три таблиці, enum `admin_role`.
...
**Auth:** none. **Throttle:** 5/хв за IP + за email (FR-006).
...
`ADMIN_JWT_EXPIRES_IN` — 15 хвилин, як у клієнта. Це верхня межа затримки
відкликання (SC-003): деактивований адмін живе рівно стільки, скільки лишилося
його access-токену, бо перевіряти `is_active` в базі на кожен запит — це
запит до бази на кожен запит.
...
- **Ротація refresh** — кожне поновлення видає нову пару і гасить попередню;
  ...
  Пом'якшення — вікно пільги 10 секунд, протягом якого щойно витрачений
  токен ще віддає ту саму нову пару, замість відкликати ланцюжок.
