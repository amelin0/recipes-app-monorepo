...
> **Status is `Approved`, not `Implemented`:** рекомендовані значення в
> редакторі нутрієнта (FR-003a) чекають на `GET /profile/recommendations`, який
> зʼявиться з онбордингом. Збереження і читання цілі працюють.
...
- **FR-003a, рекомендовані значення** в редакторі нутрієнта. Потребують
  серверних BMR/TDEE, які рахуються з профілю (стать, вік, зріст, вага,
  активність) — а ці поля додає онбординг.
