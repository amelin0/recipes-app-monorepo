...
- `403` `auth.email-not-verified` — пароль правильний, але email не
  ...
  нічого не розкриває тому, хто акаунтом не володіє. У непідтвердженого
  акаунту пароля в `users` ще немає — пароль звіряється з тим, що несе його
  код підтвердження (`otp_codes.password_hash`, див. sign-up plan), і новий
  код переносить його далі.
- `403` `auth.account-blocked` — пароль правильний, акаунт заблоковано.
...
- `403` `auth.account-blocked`.
- `409` `common.conflict` — лише якщо паралельні входи тієї самої людини
  програли гонку тричі поспіль (див. нижче); на практиці недосяжно.
...
2. Адреса вже має акаунт → **прив'язуємо** ідентичність (FR-006). Інакше той,
   ...
   що просив довести код на пошті. **Разом із цим, в одній транзакції
   (`UserRepository.linkOAuthIdentity`), акаунт втрачає пароль і коди
   підтвердження**: їх задав той, хто адресою не володів, — інакше сторонній,
   що зареєстрував чужу адресу першим, мав би пароль від акаунту, який власник
   щойно підтвердив через Google. Власник задає пароль через відновлення
   (sign-up FR-013). Підтверджений акаунт не змінюється.
3. Нікого немає → створюємо акаунт **підтверджений і без пароля**
   (sign-up FR-012) — **разом з ідентичністю, в одній транзакції**.

**Подвійне натискання.** Два перші входи однієї людини обидва бачать «нікого
немає» і вставляють; `users_email_unique` чи
`oauth_identities_provider_user_unique` відбиває того, хто програв, і він
просто проходить `resolveAccount` ще раз і знаходить записане переможцем
(не більше трьох проходів). Раніше це була 500.
...
- Прив'язка до наявного акаунту не питає підтвердження власника, бо провайдер
  ...
  на пошті.
- **Сесія зберігається під блокуванням рядка акаунту** (`FOR SHARE`, див.
  session plan) з повторною перевіркою блокування і хешу пароля, який щойно
  звірили: блокування чи скидання пароля, що закомітились після читання
  акаунту, виграють у входу.
...
- `apps/client-api/test/auth-pending-password.db-spec.ts` — вхід до
  підтвердження звіряє пароль із кодом; вхід через Google скидає пароль
  непідтвердженої чужої реєстрації.
- `apps/client-api/test/auth-registration-races.db-spec.ts` — паралельні перші
  входи через провайдера (один акаунт, одна ідентичність), паралельна
  прив'язка, Apple і Google одночасно для нової адреси.
