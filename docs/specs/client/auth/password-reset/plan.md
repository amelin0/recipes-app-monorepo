...
$sub AND consumed_at IS NULL AND expires_at > now()` (нуль рядків ⇒ 401) →
  новий `password_hash` → видалити всі refresh-токени, усі дозволи і **всі**
  коди акаунту (обох флоу). Раніше дозвіл читали, перевіряли в сервісі й
  позначали окремим запитом — два запити з одним дозволом обидва міняли
  пароль.
