...
спливлим `scheduled_for` і стирало акаунт із даними, проставляючи `executed_at`.
Потрібне рішення, чого саме воно торкається: `ON DELETE CASCADE` прибере всі
залежні рядки разом із `users`, але чи має лишатися анонімізований слід (для
фінансових звітів по підписках, наприклад) — питання, яке краще вирішити
разом із доменом subscription. До того ж завдання потребує runbook.
