...
**Видалення найсвіжішої ваги чи зросту повертає профіль до попереднього
виміру** — у тій самій транзакції й під тим самим блокуванням профілю, що й
запис, тож інваріант «профіль = найсвіжіший вимір» тримається для обох
операцій і їхніх гонок. Якщо вимірів цього показника не лишилось, профіль
зберігає поточне значення: відповідь анкети, яку вимір замінив, ніде більше
не лежить, а `null` забрав би рекомендацію.
...
- `apps/client-api/test/progress.db-spec.ts` — запис і поява на картці,
  ...
  профіль дорівнює `current` картки; те саме для зросту. Видалення
  найсвіжішого повертає профіль до попереднього, видалення єдиного лишає
  профіль як є, паралельні видалення й запис лишають профіль на
  найсвіжішому з тих, що вижили.
