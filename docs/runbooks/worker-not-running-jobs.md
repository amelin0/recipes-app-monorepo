...
- в інбоксі видно сповіщення, старші за 90 днів (строк зберігання —
  `NOTIFICATION_RETENTION_DAYS`, їх прибирає та сама нічна задача);
- користувачі не отримують «преміум незабаром закінчиться», хоча підписки
  добігають кінця.

Це **не** аварія: жоден користувацький запит від цього не падає. Але три речі
тихо деградують — таблиці ростуть, інбокс тримає сповіщення довше за
обіцяний строк, а люди дізнаються про кінець підписки від застосунку, а не
від нас.
...
(`{"refreshTokens":N,"otpCodes":N,"passwordResetPermits":N,"notifications":N}`,
`{"expired":N,"warned":N}` або `{"overdueDeletions":N,...}`). Провал —
...
# і сповіщень поза строком зберігання теж (0)
psql "$DATABASE_URL" -c "select count(*) from notifications where created_at < now() - interval '90 days';"
```

## Escalation

Воркер можна лишити зупиненим на кілька днів без наслідків для користувачів:
жоден запит через нього не проходить. Що росте — таблиці токенів, кодів і
сповіщень; що не працює — попередження про закінчення підписки. Тобто це
робота на понеділок, а не на ніч. Коли воркер повернеться, перша ж ніч
забере весь накопичений хвіст сповіщень — батчами по тисячі, тож окремих
дій не потрібно.
