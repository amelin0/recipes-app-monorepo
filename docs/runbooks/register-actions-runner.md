...
Навіть як служба раннер може лягти тихо (2026-09-15: `enabled`, але
`inactive` після перезавантаження — три пуші простояли в черзі годинами, і
жодного сигналу, бо всі сповіщення шле `deploy.sh`, який без раннера не
запускається). Ватчдог що 10 хвилин піднімає мертвий юніт і рестартить
залиплий listener; раннер посеред job'а не чіпає:

```bash
sudo ./infra/prod/install-runner-watchdog.sh
```

Ідемпотентний — переганяйте після зміни будь-якого файлу в
`infra/prod/runner-watchdog/`. Лог: `/home/actions/actions-runner/watchdog.log`.
