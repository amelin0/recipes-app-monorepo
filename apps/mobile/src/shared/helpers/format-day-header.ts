...
/**
 * Format a date as `Понеділок, 11 травня` (Figma Home header, 896:24594):
 * capitalized weekday + day + lower-case month. Day and month are formatted
 * together so the locale picks the correct grammatical case (uk: «11 травня»,
 * not «11 травень»).
 *
 * The «Додати продукт» subtitle capitalizes the month instead — see
 * {@link formatDayHeaderAccusative}. The design is inconsistent between the two
 * screens; each follows its own frame.
 */
