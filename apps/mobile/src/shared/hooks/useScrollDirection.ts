...
/**
 * Tracks the active scroll direction on the UI thread for sticky-
 * header / pull-to-refresh logic.
 *
 * `param='include-negative'` keeps negative offsets (iOS bounce)
 * in the comparison instead of clamping at 0 — needed when consumers
 * react to **over-scroll** above the top edge (pull-to-refresh).
 */
export function useScrollDirection(param?: 'include-negative') {
    ...
    const onScroll = (e: ReanimatedScrollEvent | number) => {
        ...
        const positivePrevOffsetY = param === 'include-negative' ? prevOffsetY.get() : Math.max(prevOffsetY.get(), 0);
