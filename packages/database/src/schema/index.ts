// Every table drizzle-kit should migrate must be exported here —
// `drizzle.config.ts` points at this barrel.
export * from './users.schema';
export * from './refresh-tokens.schema';
export * from './otp-codes.schema';
export * from './password-reset-permits.schema';
export * from './oauth-identities.schema';
export * from './profiles.schema';
export * from './user-settings.schema';
export * from './user-reminders.schema';
export * from './account-deletion-requests.schema';
export * from './feedback.schema';
export * from './nutrition-goals.schema';
export * from './meal-log-entries.schema';
export * from './water-log-entries.schema';
export * from './daily-steps.schema';
export * from './body-measurements.schema';
export * from './dish-categories.schema';
export * from './cuisines.schema';
export * from './diets.schema';
export * from './product-groups.schema';
export * from './products.schema';
export * from './recipes.schema';
export * from './recipe-ingredients.schema';
export * from './recipe-steps.schema';
export * from './recipe-diets.schema';
export * from './recipe-favorites.schema';
export * from './meal-plan-items.schema';
