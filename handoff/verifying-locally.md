...
**Зріз «відкликання сесії» (2026-09-12), міграція `0025`.** Повний клієнтський
суїт не ганявся — 30+ хвилин, і зміна його не зачіпає цілком. Прогнано те, що
зачіпає:

| Команда                                                            | Результат |
| ------------------------------------------------------------------ | --------- |
| `pnpm exec jest --config jest.db.config.ts "auth-\|user-profile"` (client-api) | **88/88**, 11 суїтів — разом із новим `auth-session-revocation` (9) |
| `pnpm exec jest --config jest.db.config.ts admin-auth` (admin-api)  | **44/44** — 40 старих плюс 4 нових |

Обидва нові блоки спершу були червоними (5 з 9 і 2 з 4) — інакше вони не
доводять, що баг існував.
