...
**Останнє оновлення:** 2026-09-07 — зріз 12 (сповіщення і FAQ) закрито.
...
- **`meal-plan` FR-006** — кошик на страві й «Додати до списку покупок»
  ...
  не окремий зріз.
- **`client/subscription`** — найбільший із того, що лишилось (15 FR) і
  єдиний, що впирається в зовнішні сервіси: валідація чеків App Store і
  Google Play. Він же розблокує `referral` (статистика реферала — це
  зароблені місяці підписки) і тип сповіщень про підписку.
- **`client/user/referral`** — три з пʼяти FR (код, копіювання, шаринг)
  можна зробити зараз; статистика чекає на підписку.
- **`system/error-screen`** серверної частини не має взагалі — форма
  звернення вже існує.
- Далі admin-зрізи; їм спершу потрібні специфікації — у `docs/specs/admin/`
  зараз лише README.
...
| `pnpm --filter @dns/client-api test:db`      | 167 — плюс сповіщення і FAQ (треба docker)                                           |
