...
**Останнє оновлення:** 2026-09-07 — зріз 11 (список покупок) закрито,
...
- **`meal-plan` FR-006** — кошик на страві й «Додати до списку покупок»
  ...
  не окремий зріз.
- Далі: `subscription`, `notifications`, `referral`, `faq`, `error-screen`,
  потім admin-зрізи (їм спершу потрібні специфікації: у `docs/specs/admin/`
  зараз лише README).
...
| `pnpm --filter @dns/client-api test:db`      | 153 — auth, user, nutrition, анкета, прогрес, каталог, план і список (треба docker)  |
