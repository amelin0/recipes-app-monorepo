...
**Останнє оновлення:** 2026-09-07 — зріз 9 (створення власної страви)
закрито, перевірено на живій базі.
...
- **`client/meal-plan`** — розблокує слоти раціону на головному екрані
  (daily-tracking FR-006) і режим пікера в пошуку (recipe-search FR-009).
...
| `pnpm --filter @dns/validation test`         | 41 — плюс запити каталогу, групи фільтрів, власний продукт і власна страва           |
...
| `pnpm --filter @dns/client-api test:db`      | 119 — auth, user, звернення, nutrition, анкета, прогрес і каталог (треба docker)     |
