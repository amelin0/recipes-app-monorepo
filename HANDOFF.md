...
**Останнє оновлення:** 2026-09-07 — зріз 8 (каталог рецептів і продуктів)
...
- **`client/recipe/create-dish`** — запис власної страви: форма, кроки,
  розрахунок КБЖВ зі складу, фото через presigned upload, таблиця звʼязку
  «інгредієнти кроку». Каталог його вже чекає: плитка «додати рецепт» веде
  в нікуди, а `POST /products` уже є, але жоден екран його не викликає.
...
- **`packages/database` тепер залежить від `@dns/constants`.** Резолюція
  ...
  нульова.
- **`POST /products` випереджає свій екран.** Форма додавання продукту
  належить `create-dish`; ендпоінт збудований зараз, бо пікер складу без
  нього впирається у стелю каталогу.
...
| `pnpm --filter @dns/validation test`         | 36 — плюс запити каталогу, групи фільтрів і власний продукт                          |
...
| `pnpm --filter @dns/client-api test:db`      | 106 — auth, user, звернення, nutrition, анкета, прогрес і каталог (треба docker)     |
